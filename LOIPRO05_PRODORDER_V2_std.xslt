...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="/LOIPRO05/IDOC/E1AFKOL">
        <ManufacturingOrder>
            ...
            <ChangeProduction>
                <SupportChangeAfterRelease>false</SupportChangeAfterRelease>
                <HoldProduction>BY_SFC</HoldProduction>
                <DataCollectionCopy>false</DataCollectionCopy>
                <QuantityCopy>false</QuantityCopy>
                <ActivityConfirmationCopy>false</ActivityConfirmationCopy>
    ...
    <xsl:template name="getMaterial">
        ...
        <xsl:value-of select="
            if (matches($materialString, '^\d+$')) then
              format-number(xs:decimal($materialString), $materialMask)
            else
              $materialString
        "/>
    </xsl:template>
