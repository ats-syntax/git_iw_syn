...
<!-- #### CHANGE LOG #### -->
<!-- CHG001: Enable Dynamic Order Change, Amadeus Tschorn, 13.05.2024 -->
<!-- CHG002: Enable AutoScheduling with Dynamic Order Change, Amadeus Tschorn, 26.08.2024 -->
...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="/LOIPRO05/IDOC/E1AFKOL">
		...
		<!-- CHG003 Amadeus Tschorn @> -->
        <xsl:variable name="customerOrderName">
            <xsl:choose>
                <xsl:when test="//IDOC/E1AFKOL/E1AFPOL/NAME1 != ''">
                    <xsl:value-of select="//IDOC/E1AFKOL/E1AFPOL/NAME1"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="''"/>
                </xsl:otherwise>
        ...
        <ManufacturingOrder>
            ...
            <!-- Sample to specify build quantity -->
            <!--
            <BuildQuantityInBaseUnit>
                <xsl:value-of select="BMENGE"/>
            </BuildQuantityInBaseUnit>
            -->
            ...
            <xsl:if test="E1AFLTH">
                <LongText pp:MIMECode="text/plain" pp:TextCategory="{E1AFLTH[1]/TDTEXTTYPE}">
                    ...
                    <xsl:for-each select="E1AFLTH[1]/E1AFLTP">
                        <xsl:if test="position() != 1 and TDFORMAT = '*'"><xsl:text>&#xa;</xsl:text></xsl:if>
            ...
            <xsl:for-each select="E1AFFLL[FLGAT != '2']">
			...
			<!--@> Amadeus Tschorn #### 28.05.2024-->
			<CustomFieldList>
				<CustomField>
					<Attribute>WBS</Attribute>
					<Value>
						<xsl:value-of select="//IDOC/E1AFKOL/PSPEL"/>
					</Value>
				</CustomField>
				<!-- CHG007 Amadeus Tschorn @> -->
				<CustomField>
					<Attribute>FPR</Attribute>
					<Value>
						<xsl:value-of select="//IDOC/E1AFKOL/PSPEL"/>
					</Value>
				</CustomField>
				<!-- <@ Amadeus Tschorn CHG007 -->
				<!-- CHG004 Amadeus Tschorn @> -->
				<CustomField>
					<Attribute>CUSTOMER_NAME</Attribute>
					<xsl:choose>
						<xsl:when test="$customerOrderName != ''">
							<Value>
								<xsl:value-of select="//IDOC/E1AFKOL/E1AFPOL/NAME1"/>
							</Value>
						</xsl:when>
				...
				<!-- <@ Amadeus Tschorn CHG004 -->
			</CustomFieldList>
			<!--Amadeus Tschorn #### 28.05.2024<@-->
			<!--@> Amadeus Tschorn #### 28.05.2024-->
			<BomHeaderCustomFieldList>
                <CustomField>
                    <Attribute>WBS_BOM</Attribute>
                    <Value>
						<xsl:value-of select="//IDOC/E1AFKOL/PSPEL"/>
					</Value>
                </CustomField>
            ...
            <ChangeProduction>
				<!-- CHG001 Amadeus Tschorn @> -->
                <SupportChangeAfterRelease>true</SupportChangeAfterRelease>
                <!--HoldProduction>BY_SFC</HoldProduction-->
                <DataCollectionCopy>true</DataCollectionCopy>
                <QuantityCopy>true</QuantityCopy>

                <!--
                Uncomment <AutoTriggerChangeProduction> section to enable auto trigger change production.
                If RoutingStepID is provided, PlaceSFCIn will be ignored.
                Optional values for PlaceSFCIn: FIRST, FIRST_UNCOMPLETED.
                -->
                <!-- CHG006 Amadeus Tschorn @> -->
                <!--AutoTriggerChangeProduction>
                    <PlaceSFCIn>
                        <RoutingStepID></RoutingStepID>
                        <PlaceSFCIn>FIRST_UNCOMPLETED</PlaceSFCIn>
                    </PlaceSFCIn>
                </AutoTriggerChangeProduction-->
				<!-- <@ Amadeus Tschorn CHG006 -->
                
				<!-- CHG002 Amadeus Tschorn @> -->
                <xsl:choose>
					<xsl:when test="/LOIPRO05/IDOC/E1AFKOL/WERKS=''">
						<AutoScheduling>true</AutoScheduling>		
					</xsl:when>
					<xsl:otherwise>
						<AutoScheduling>false</AutoScheduling>		
					</xsl:otherwise>
				</xsl:choose>
				<!-- <@ Amadeus Tschorn CHG002 -->
            </ChangeProduction>
            <!-- Sample to customize retain DM schedules flag -->
            <!--
			<RetainSchedules>true</RetainSchedules>
            -->			
			<!-- CHG005 Amadeus Tschorn @> -->
			<xsl:choose>
				<xsl:when test="/LOIPRO05/IDOC/E1AFKOL/WERKS=''">
					<RetainSchedules>false</RetainSchedules>		
				</xsl:when>
				<xsl:otherwise>
					<RetainSchedules>true</RetainSchedules>		
				</xsl:otherwise>
			</xsl:choose>							
			<!-- <@ Amadeus Tschorn CHG005 -->
			
            <!-- Sample to customize quantity validation flag on routing -->
            <!--
            <QuantityValidation>
            ...
            -->
            <!-- Sample to customize relaxed flow flag on routing
            <RelaxedFlow>
                <xsl:value-of select="false()"/>
            </RelaxedFlow>
            -->
            <!-- Sample to customize serialized Sfc Pattern Key
            <SerializedSfcPatternKey>DEFAULT</SerializedSfcPatternKey>
            -->
        </ManufacturingOrder>
        ...
        <workInstructionListIn>
            ...
            <xsl:for-each select="E1AFDFH/E1AFDHO">
                <workInstructionIn>
                    <trackViewing>true</trackViewing>
                    <workInstruction>
            ...
            <xsl:for-each select="E1AFFLL/E1AFVOL[not(E1AFVOL2/NOT_MES_REL = 'X')]">
                <!-- Step id format logic:
                Except all alternative sequences, if only standard sequence exist, use VORNR, otherwise use PLNFL-VORNR
                -->
                <xsl:variable name="routingStepId">
                    <xsl:choose>
                        <xsl:when test="count(//E1AFFLL[FLGAT != '2']) = 1">
                            <xsl:value-of select="VORNR"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="concat(../PLNFL, '-', VORNR)"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>	   
                                        ...
                                        <routingStepRef>
                                            <stepId>
                                                <xsl:choose>
                                                    ...
                                                    <xsl:otherwise>
                                                        <xsl:value-of select="$routingStepId"/>
                                                    </xsl:otherwise>
                                                </xsl:choose>
                                            </stepId>
    ...
    <xsl:template name="sequenceTemplate">
        <!-- current context is E1AFFLL -->
        <xsl:for-each select="E1AFVOL[E1JSTVL[last()]/STAT!='I0013']">
            <ManufacturingOrderActivityNetworkElement>
                ...
                <pp:MfgOrderNodeID>
                    <xsl:value-of select="concat('SEQ_', ../PLNFL, '_OP_', VORNR)"/>
                </pp:MfgOrderNodeID>
                ...
                <xsl:for-each select="E1RESBL">
                    <xsl:if test="not(DUMPS = 'X')">													
                    <pp:ManufacturingOrderComponent>
                            <ComponentType>
                                <xsl:value-of select="DUMPS"/>
                            </ComponentType>										   
                        <RequirementType>
                        ...
                        <RequiredQuantityInBaseUnit pp:unitCode="{MEINS}">
                            ...
                            <!-- QuantityInBaseUnit is the quantity of the BOM component required for each unit of the
                            finished product. If provided, this value will be used directly by the system instead of
                            calculating it from the total quantity divided by the order quantity. -->
                            <!--
                            <QuantityInBaseUnit>
                                <xsl:value-of select="format-number(floor((BDMNG div ../../../GAMNG) * 1000000) div 1000000, '0.######')"/>
                            </QuantityInBaseUnit>
                            -->
                        ...
                        <xsl:for-each select="E1STPUL">
                            <ZBOMSubItem>
                                <ZBOMSubItemNumberValue><xsl:value-of select="UPOSZ"/></ZBOMSubItemNumberValue>
                                <ZBOMSubItemInstallationPoint><xsl:value-of select="EBORT"/></ZBOMSubItemInstallationPoint>
                                <ZBillOfMaterialSubItemText><xsl:value-of select="UPTXT"/></ZBillOfMaterialSubItemText>
                                <ZBillOfMaterialSubItemQuantity><xsl:value-of select="UPMNG"/></ZBillOfMaterialSubItemQuantity>
                                    <!-- It is the quantity of the sub component required for each unit of the
                                    finished product. If provided, this value will be used directly by the system instead of
                                    calculating it from the total quantity divided by the order quantity. -->
                                    <!--
                                    <ZBillOfMaterialSubItemCalculatedQuantity></ZBillOfMaterialSubItemCalculatedQuantity>
                                    -->
                            </ZBOMSubItem>
                        </xsl:for-each>
                ...
                <xsl:for-each select="E1QAMVL">
                    <pp:ManufacturingOrderInspCharc>
						...
						<!-- Unknown customizing -->
                        <!--<InspCharacteristicSampleSize pp:unitCode="{SMPL_UNIT}">3</InspCharacteristicSampleSize>-->
                        <InspectionScope>
				...
				<CustomFieldList>
                    <CustomField>
                        <Attribute>SYM_KEY</Attribute>
						<!--KW/14.02.25: Replacement of whitespaces-->
                        <Value><xsl:value-of select="translate(E1AFVOL2/USR03, ' ', '')"/></Value>
                    </CustomField>
					<CustomField>
                        <Attribute>REFERENCE_SEQUENCE</Attribute>
                        <Value><xsl:value-of select="../REFERENCE_SEQUENCE"/></Value>
                    </CustomField>
					<CustomField>
                        <Attribute>STEP_SEQUENCE_TYPE</Attribute>
                        <Value><xsl:value-of select="../FLGAT"/></Value>
                    </CustomField>
					<CustomField>
						<Attribute>BRANCH_OPERATION</Attribute>
						<Value>
							<xsl:choose>
								<xsl:when test="../VORNR1">
					...
					<CustomField>
						<Attribute>RETURN_OPERATION</Attribute>
						<Value>
							<xsl:choose>
								<xsl:when test="../VORNR2">
                ...
                <!-- Sample to customize resource type
            ...
            <xsl:for-each select="E1AFUVL[E1JSTUL[last()]/STAT!='I0013']">
                <ManufacturingOrderActivityNetworkElement>
                    ...
                    <pp:MfgOrderNodeID>
                        <xsl:value-of select="concat('SEQ_', ../../PLNFL, '_SUBOP_', ../VORNR, '-', UVORN)" />
                    </pp:MfgOrderNodeID>
                    <pp:OrderInternalBillOfOperations />
                    <pp:OrderIntBillOfOperationsItem>
        ...
        <xsl:variable name="refSequence">
            <xsl:choose>
                <xsl:when test="string(REFERENCE_SEQUENCE) != ''">
                    <xsl:value-of select="number(REFERENCE_SEQUENCE)"/>
                </xsl:when>
                <xsl:otherwise>
                    <!-- if REFERENCE_SEQUENCE not present, use the standard sequence number -->
                    <xsl:value-of select="../E1AFFLL[FLGAT = '0']/PLNFL"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>

        <xsl:variable name="startingNode">
            <xsl:call-template name="getStartingNode">
                <xsl:with-param name="refSequence" select="$refSequence"/>
                <xsl:with-param name="currentSequence" select="."/>
            </xsl:call-template>						 
        </xsl:variable>

        <xsl:variable name="endingNode">
            <xsl:call-template name="getEndingNode">
                <xsl:with-param name="refSequence" select="$refSequence"/>
                <xsl:with-param name="currentSequence" select="."/>
            </xsl:call-template>						 
        </xsl:variable>
        ...
        <xsl:for-each select="E1AFVOL[E1JSTVL[last()]/STAT!='I0013']">
            ...
            <xsl:choose>
                <xsl:when test="position() = 1">
                    <!-- first operation of current sequence -->
                    <ManufacturingOrderRelationship>
                        <xsl:copy-of select="$startingNode"/>
                ...
                <xsl:otherwise>
                    <ManufacturingOrderRelationship>
                        ...
                        <PredecessorMfgOrderNodeID>
                            <xsl:value-of select="concat('SEQ_', ../PLNFL, '_OP_', preceding-sibling::E1AFVOL[E1JSTVL[last()]/STAT!='I0013'][1]/VORNR)"/>
                        </PredecessorMfgOrderNodeID>
                        <SuccessorMfgOrderNodeType>Operation</SuccessorMfgOrderNodeType>
                        <SuccessorMfgOrderNodeID>
                            <xsl:value-of select="concat('SEQ_', ../PLNFL, '_OP_', VORNR)"/>
    ...
    <xsl:template name="getStartingNode">
        <xsl:param name="refSequence"/>
        <xsl:param name="currentSequence"/>

        <xsl:choose>
            <xsl:when test="$currentSequence/FLGAT = '0' or string($currentSequence/VORNR1) = ''">
                <!-- for standard sequence, or sequence w/o branch operation, starting node would be StartEvent -->
                <PredecessorMfgOrderNodeType>StartEvent</PredecessorMfgOrderNodeType>
                <PredecessorMfgOrderNodeID>
                    <xsl:value-of select="concat(//IDOC/E1AFKOL/AUFNR, 'SEVT')"/>
                </PredecessorMfgOrderNodeID>
            </xsl:when>
            <xsl:otherwise>
                <!-- for other sequence, find the node before the branch operation -->
                <xsl:variable name="branchOp" select="$currentSequence/VORNR1"/>
                <xsl:variable name="refSequenceSegment" select="//IDOC/E1AFKOL/E1AFFLL[PLNFL = $refSequence]"/>

                <xsl:for-each select="$refSequenceSegment/E1AFVOL[E1JSTVL[last()]/STAT!='I0013']">
                    <xsl:sort select="VORNR"/>
                    <xsl:if test="VORNR = $branchOp">
                        <xsl:choose>
                            <xsl:when test="$refSequenceSegment/FLGAT = '0' and empty(preceding-sibling::E1AFVOL[E1JSTVL[last()]/STAT!='I0013'])">
                                <!-- no operations before the branch operation and the referred sequence is standard, use StartEvent -->
                                <PredecessorMfgOrderNodeType>StartEvent</PredecessorMfgOrderNodeType>
                                <PredecessorMfgOrderNodeID>
                                    <xsl:value-of select="concat(//LOIPRO05/IDOC/E1AFKOL/AUFNR, 'SEVT')"/>
                                </PredecessorMfgOrderNodeID>
                            </xsl:when>
                            <xsl:when test="empty(preceding-sibling::E1AFVOL[E1JSTVL[last()]/STAT!='I0013'])">
                                <!-- no operations before the branch operation and referred sequence is not standard -->
                                <!-- continue to find the node before the branch operation in the referred sequence -->
                                <xsl:call-template name="getStartingNode">
                                    <xsl:with-param name="refSequence">
                                        <xsl:choose>
                                            <xsl:when test="string($refSequenceSegment/REFERENCE_SEQUENCE) != ''">
                                                <xsl:value-of select="number($refSequenceSegment/REFERENCE_SEQUENCE)"/>
                                            </xsl:when>
                                            <xsl:otherwise>
                                                <!-- if REFERENCE_SEQUENCE not present, use the standard sequence number -->
                                                <xsl:value-of select="//LOIPRO05/IDOC/E1AFKOL/E1AFFLL[FLGAT = '0']/PLNFL"/>
                                            </xsl:otherwise>
                                        </xsl:choose>
                                    </xsl:with-param>
                                    <xsl:with-param name="currentSequence" select="$refSequenceSegment"/>
                                </xsl:call-template>
                            </xsl:when>
                            <xsl:otherwise>
                                <PredecessorMfgOrderNodeType>Operation</PredecessorMfgOrderNodeType>
    ...
    <xsl:template name="getEndingNode">
        <xsl:param name="refSequence"/>
        <xsl:param name="currentSequence"/>

        <xsl:choose>
            <xsl:when test="$currentSequence/FLGAT = '0'">
                <!-- for standard sequence, ending node would be EndEvent -->
                <SuccessorMfgOrderNodeType>EndEvent</SuccessorMfgOrderNodeType>
                <SuccessorMfgOrderNodeID>
                    <xsl:value-of select="concat(//IDOC/E1AFKOL/AUFNR, 'EEVT')"/>
                </SuccessorMfgOrderNodeID>
            </xsl:when>
            <xsl:otherwise>
                <!-- for other sequence, find the node after the return operation -->
                <xsl:variable name="returnOp" select="$currentSequence/VORNR2"/>
                <xsl:variable name="refSequenceSegment" select="//IDOC/E1AFKOL/E1AFFLL[PLNFL = $refSequence]"/>

                <xsl:for-each select="$refSequenceSegment/E1AFVOL[E1JSTVL[last()]/STAT!='I0013']">
                    <xsl:sort select="VORNR"/>
                    <xsl:if test="VORNR = $returnOp">
                        <xsl:choose>
                            <xsl:when test="$refSequenceSegment/FLGAT = '0' and empty(following-sibling::E1AFVOL[E1JSTVL[last()]/STAT!='I0013'])">
                                <!-- no operations after the return operation and the referred sequence is standard, use EndEvent -->
                                <SuccessorMfgOrderNodeType>EndEvent</SuccessorMfgOrderNodeType>
                                <SuccessorMfgOrderNodeID>
                                    <xsl:value-of select="concat(//IDOC/E1AFKOL/AUFNR, 'EEVT')"/>
                                </SuccessorMfgOrderNodeID>
                            </xsl:when>
                            <xsl:when test="empty(following-sibling::E1AFVOL[E1JSTVL[last()]/STAT!='I0013'])">
                                <!-- no operations after the return operation and referred sequence is not standard -->
                                <!-- continue to find the node after the return operation in the referred sequence -->
                                <xsl:call-template name="getEndingNode">
                                    <xsl:with-param name="refSequence">
                                        <xsl:choose>
                                            <xsl:when test="string($refSequenceSegment/REFERENCE_SEQUENCE) != ''">
                                            ...
                                            <xsl:otherwise>
                                                <!-- if REFERENCE_SEQUENCE not present, use the standard sequence number -->
                                                <xsl:value-of select="//IDOC/E1AFKOL/E1AFFLL[FLGAT = '0']/PLNFL"/>
                                            </xsl:otherwise>
                                        </xsl:choose>
                                    </xsl:with-param>
                                    <xsl:with-param name="currentSequence" select="$refSequenceSegment"/>
                            ...
                            <xsl:otherwise>
                                <SuccessorMfgOrderNodeType>Operation</SuccessorMfgOrderNodeType>
                                <SuccessorMfgOrderNodeID>
                                    <xsl:value-of select="concat('SEQ_', ../PLNFL, '_OP_', following-sibling::E1AFVOL[E1JSTVL[last()]/STAT!='I0013'][1]/VORNR)"/>
                                </SuccessorMfgOrderNodeID>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:if>
