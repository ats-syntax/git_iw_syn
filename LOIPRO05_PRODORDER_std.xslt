...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/LOIPRO05">
        <productionOrderIn>
            ...
            <!--inspectionLot -->
            <inspectionLot>
                <xsl:variable name="INSPECTION_LOT" select="IDOC/E1AFKOL/PRUEFLOS"/>
                <xsl:if test="$INSPECTION_LOT != '' and number($INSPECTION_LOT) != 0">
                    <xsl:value-of select="substring(concat('000000000000',$INSPECTION_LOT),string-length($INSPECTION_LOT)+1)"/>
                </xsl:if>
            </inspectionLot>
            ...
            <shopOrderIn>
                ...
                <xsl:if test="IDOC/E1AFKOL/E1AFLTH != ''">
                    <shopOrderHeaderTextDTOList>
                        <language>
                            <xsl:call-template name="convertLanguageCode">
                                <xsl:with-param name="spras" select="IDOC/E1AFKOL/E1AFLTH/TDSPRAS"/>
                            </xsl:call-template>
                        </language>
                        <xsl:for-each select="IDOC/E1AFKOL/E1AFLTH/E1AFLTP">
                            <shopOrderHeaderTextDTO>
                                <tdFormat>
                ...
                <overDeliveryIsAllowed>
                    <xsl:call-template name="convertToBool">
                        <xsl:with-param name="bool" select="IDOC/E1AFKOL/E1AFPOL/UEBTK"/>
                    </xsl:call-template>
                </overDeliveryIsAllowed>
                        ...
                        <bomComponentDTOList>
                            <xsl:for-each select="IDOC/E1AFKOL/E1AFFLL/E1AFVOL/E1RESBL">
                                ...
                                <xsl:variable name="totalQty">
                                    <xsl:choose>
                                        <xsl:when test="string(ALPGR) and number(EWAHR) = 0">
                                ...
                                <xsl:if test = "string(MATNR|MATNR_EXTERNAL|MATNR_LONG) and $totalQty > 0 and number(AFPOS)!=1 and not($phType = 'X')">
                                    <bomComponentDTO>
                                        ...
                                        <totalQtyBaseUom>
                                            <xsl:choose>
                                                <xsl:when test="SHKZG = 'S'">
                                                    <xsl:value-of select="concat('-', $totalQty)"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                        ...
                                        <xsl:choose>
                                            <xsl:when test="BACKFLUSH='X'">
                                                <materialCompIsMarkedForBackflush>true</materialCompIsMarkedForBackflush>
                                            </xsl:when>
                                            <xsl:otherwise>
                                                <materialCompIsMarkedForBackflush>false</materialCompIsMarkedForBackflush>
                                            </xsl:otherwise>
                                        </xsl:choose>
                                        <!-- <assembleAsRequired></assembleAsRequired>-->
                                    </bomComponentDTO>
                                </xsl:if>
                            </xsl:for-each>
                        </bomComponentDTOList>
                                ...
                                <xsl:if test="FLGAT = 0">
                                    <xsl:for-each select="E1AFVOL[E1JSTVL[last()]/STAT!='I0013']">
                                        ...
                                        <routingStepDTO>
                                            ...
                                            <inspectionCharacteristicDTOList>
