					...
					<xsl:when test="string(itemList/item/material)">
						<xsl:for-each select="itemList/item">
							<item>
								...
								<ORDER_ITNO>
									<xsl:value-of select="orderItemNumber"/>
								</ORDER_ITNO>
								<RESERVATION_NUMBER>
									<xsl:value-of select="reservationNumber"/>
								</RESERVATION_NUMBER>
								<RESERVATION_ITEM>
									<xsl:value-of select="reservationItemNumber"/>
								</RESERVATION_ITEM>	
								<SUPPLY_AREA>
									<xsl:call-template name="setValue">
										<xsl:with-param name="value" select="storageLocation"/>
									</xsl:call-template>
								</SUPPLY_AREA>
								<BATCH>
									<xsl:call-template name="setValue">
								...
								<EXPIRYDATE>
									<xsl:call-template name="setDate">
										<xsl:with-param name="value" select="expiryDate"/>
									</xsl:call-template>
								</EXPIRYDATE>
								<PROD_DATE>
									<xsl:call-template name="setDate">
